...
// Delete a fragment's data and metadata from S3 and DynamoDb
// https://github.com/awsdocs/aws-sdk-for-javascript-v3/blob/main/doc_source/s3-example-creating-buckets.md#deleting-an-amazon-s3-bucket
async function deleteFragment(ownerId, id) {
  ...
  try {
    // Use our client to send the command
    await s3Client.send(s3Command); // Wait for the data to come back from AWS
    await ddbDocClient.send(dynamodbCommand);
  } catch (err) {
    const { Bucket, Key } = s3Params;
    logger.error({ err, Bucket, Key }, 'Error deleting fragment data from S3');
    logger.warn({ err, dynamodbParams }, 'error deleting fragment from DynamoDB');
    throw new Error('unable to delete fragment data from s3 and DynamoDB');
  }
}
